...
export const useVisualizationStore = defineStore('visualization', () => {
  ...
  const SubnettingMethod = (cidr: number) => {
    ...
    const subnetId = [...Object.values(classesNetworkDefault.value)]
      .find((cls) => cls.defaultCidr <= cidr && cls.endCidr >= cidr)
      ?.subnets?.split('.')
      .map((octet) => parseInt(octet));
    const indexOfZero = subnetId?.findIndex((octet) => octet === 0);
    const calculatedValue = 256 - 2 ** (32 - cidr);
    const newSubnetId =
      indexOfZero !== undefined && subnetId
        ? subnetId
          ?.slice(0, indexOfZero)
          .concat([calculatedValue, ...subnetId?.slice(indexOfZero + 1)])
        : subnetId;
  ...
  const cidrPrefixSetter = computed(() => ({
    ...
    subnetMask:
      cidrPrefix.value && cidrPrefix.value[0] === 24
        ? classesNetworkDefault.value.C.subnets && classesNetworkDefault.value.C.subnets
        : SubnettingMethod(cidrPrefix.value[0]!),
